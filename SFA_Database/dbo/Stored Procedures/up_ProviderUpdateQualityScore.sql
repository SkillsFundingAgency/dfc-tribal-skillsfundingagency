...
-- ******************************************************************************************************************
-- NOTE: There is an equivalent version of this stored procedure in the SFA_DataWarehouse database project.
...
DECLARE
	...
	,@LastActivity				DATETIME = (SELECT LastAllDataUpToDateTimeUtc FROM Provider WHERE ProviderId = @ProviderId);
...
BEGIN TRANSACTION
	...
	FROM #CourseInfo c
		...
		LEFT JOIN CourseInstanceStartDate cisd on cisd.CourseInstanceId = ci.CourseInstanceId
	WHERE 
	...
	SELECT @CoursesWithFutureStartDates = Sum(
		CASE
			...
			WHEN ci.StartDate <= @today AND dbo.GetMonthsBetween(ci.StartDate, @today) < 2 THEN 1
			WHEN ci.StartDate <= @today AND dbo.GetMonthsBetween(ci.StartDate, @today) = 2 THEN 0.5
			ELSE 0
		END
	)
	FROM (
