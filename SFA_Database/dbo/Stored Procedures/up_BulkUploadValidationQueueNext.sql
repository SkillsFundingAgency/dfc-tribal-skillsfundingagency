CREATE PROCEDURE [dbo].[up_BulkUploadValidationQueueNext] (
	@LargeFileSize INT,
	@LargeFilesAllowed BIT
)
AS
BEGIN
		...
		FROM (
			...
			FROM [dbo].[v_BulkUploadCurrentStatus] [bucs]
				JOIN [dbo].[BulkUploadStatus] [bus]
					...
					AND [bus].[BulkUploadStatusName] = 'Unvalidated'
			WHERE [bucs].[ProcessingToken] IS NULL
				AND [bucs].[FileSize] >= @LargeFileSize
