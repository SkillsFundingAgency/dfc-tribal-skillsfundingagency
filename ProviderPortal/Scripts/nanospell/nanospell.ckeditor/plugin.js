    ...
    CKEDITOR.plugins.add("nanospell",
    {
        ...
        init: function(editor) {
            ...
            function setUpContextMenu(editor, path) {
                    ...
                    return {
                        ...
                        onClick: function() {
                            if (suggestion.indexOf(String.fromCharCode(160)) > -1) {
                                return window
                                    .open("http://ckeditor-spellcheck.nanospell.com/license?discount=developer_max");
                            }
                ...
                editor.contextMenu.addListener(function(element) {
                    if (!element.$ ||
                        ...
                        element.$.className !== "nanospell-typo") {
                        return;
                    }
                    ...
                    if (suggestions.length == 0) {
                        ...
                        for (var i = 0; i < suggestions.length; i++) {
                            ...
                            editor.addMenuItem("nanopell_sug_" + i,
                                generateSuggestionMenuItem(word, !!!i, typo, element));
                    ...
                    //
                    if (localStorage && suggestions.length > 0) {
            ...
            function wordsToRPC(words, lang) {
                return '{"id":"c0","method":"spellcheck","params":{"lang":"' +
                    lang +
                    '","words":["' +
                    words.join('","') +
                    '"]}}';
            ...
            function parseRpc(data, words) {
                try {
                    ...
                    var msg = ("Nanospell need to be installed correctly before use (server:" +
                        settings.server +
            ...
            function wordTokenizer(singleton) {
                ...
                var validword = "[^" +
                    ...
                    invalidchar +
                    "'\u2018\u2019]";
                var result = new RegExp("(" + email + ")|(" + protocol + ")|(" + domain + ")|(" + validword + ")",
                    singleton ? "" : "g");
                if (singleton) {
                    __memtoks = result;
                } else {
            ...
            function getSuggestions(word) {
                ...
                if (suggestionscache[word] && suggestionscache[word][0]) {
                    if (suggestionscache[word][0].indexOf("*") == 0) {
                        return Array("nanospell\xA0plugin\xA0developer\xA0trial ",
                            "ckeditor-spellcheck.nanospell.com/license\xA0");
                    }
                }
        ...
        afterInit: function(editor) {
            var wiggles_filter = {
                elements: {
                    span: function(el) {
                        var isNano = !!el.attributes &&
                        (el.attributes["class"] == "nanospell-typo" ||
                            el.attributes["class"] == "nanospell-typo-disabled");
                        if (isNano) {
