<%@ Page Title="Tiny File Manager" Language="C#" AutoEventWireup="true" CodeBehind="/WebForms/FileManager/dialog.aspx.cs" Inherits="TinyFileManager.NET.dialog" %>
<%@ Import Namespace="TinyFileManager.NET" %>
...
<head>
    ...
    <link href="/Scripts/TinyFileManager.Net/css/bootstrap.min.css" rel="stylesheet" type="text/css"/>
    <link href="/Scripts/TinyFileManager.Net/css/bootstrap-lightbox.min.css" rel="stylesheet" type="text/css"/>
    <link href="/Scripts/TinyFileManager.Net/css/style.css" rel="stylesheet" type="text/css"/>
    <link href="/Scripts/TinyFileManager.Net/css/dropzone.css" type="text/css" rel="stylesheet"/>
</head>
<body>

<!----- uploader div start ------->
<div class="uploader">
    <form action="/WebForms/FileManager/dialog.aspx?cmd=upload" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
    ...
    <center>
        <button class="btn btn-large btn-primary close-uploader"><i class="icon-backward icon-white"></i> Return to files list</button>
    </center>
        ...
        <div class="span12 pull-right">
            <ul class="thumbnails ff-items">
                ...
                <%
                    ...
                    foreach (clsFileItem objF in arrLinks)
                    {
                        //get start of line html, if necessary
                        Response.Write(getStartOfLine(objF.intColNum));
                        ...
                        Response.Write(getEndOfLine(objF.intColNum));
                    }
                %>
...
<!----- lightbox div end ------->
<script type="text/javascript" src="/Scripts/TinyFileManager.Net/js/jquery.1.9.1.min.js"></script>
<script type="text/javascript" src="/Scripts/TinyFileManager.Net/js/bootstrap.min.js"></script>
<script type="text/javascript" src="/Scripts/TinyFileManager.Net/js/bootstrap-lightbox.min.js"></script>
<script type="text/javascript" src="/Scripts/TinyFileManager.Net/js/dropzone.min.js"></script>
<script language="javascript" type="text/javascript">

    var ext_img = new Array(<% Response.Write(objConfig.strAllowedImageExtensions); %>);
    var allowed_ext = new Array(<% Response.Write(strAllowedFileExt); %>);
    var track = '<% Response.Write(strEditor); %>';
    var curr_dir = '<% Response.Write(HttpUtility.JavaScriptStringEncode(strCurrPath.Replace("\\", "\\\\"))); %>';
    $(document)
        .ready(function() {
            ...
            Dropzone.options.myAwesomeDropzone = {
                ...
                maxFilesize: <% Response.Write(objConfig.intMaxUploadSizeMb); %>, // MB
            ...
            $('.new-folder')
                .click(function() {
                    ...
                    if (folder_name) {
                        ...
                        $.ajax({
                                type: "POST",
                                url: "/WebForms/FileManager/dialog.aspx?cmd=createfolder",
    ...
    function apply(file, type_file) {
        <% if (objConfig.strFillSelector != "")
           { %>
        $(<% Response.Write(objConfig.strFillSelector); %>).val(file);
        <% Response.Write(objConfig.strPopupCloseCode); %>
        <% }
           else
           { %>
