CREATE PROCEDURE [dbo].[API_CourseList_Get_v2] 
(
...
BEGIN	
	...
	IF(@CourseSearchText IS NOT NULL)
		BEGIN
			INSERT INTO @MatchingCourseResult            
				SELECT CX.[Key],
					CX.[Rank]
				FROM CONTAINSTABLE(dbo.CourseFreeText, CourseTitle, @CourseSearchText, LANGUAGE 1033) CX
				ORDER BY CX.Rank DESC;
		END;
	ELSE
		INSERT INTO @MatchingCourseResult  
	...
	;WITH RawData AS 
	 (	 
		...
		WHERE (C.QualificationTypeRef IN (SELECT Data FROM [dbo].[Split](@QualificationType, @splitChar)) OR @QualificationType IS NULL)
			AND	(C.QualificationBulkUploadRef IN (SELECT Data FROM [dbo].[Split](@QualificationLevel, @splitChar)) OR @QualificationLevel IS NULL)
			AND	(C.[LDCS1] IN (SELECT Data FROM [dbo].[Split](@LdscCategoryCode, @splitChar)) OR  
				 ...
				 C.[LDCS5] IN (SELECT Data FROM [dbo].[Split](@LdscCategoryCode, @splitChar)) OR 
				 @LdscCategoryCode IS NULL)
			--AND	(C.[IndependentLivingSkills] = @IndLivingFlg OR @IndLivingFlg = 0)
			--AND (C.[Loans24Plus] = @ErTtgStatus OR @ErTtgStatus = 0)
			--AND	(C.[OtherFundingStatus] IN (SELECT Data FROM [dbo].[Split](@OtherFundingStatus, @splitChar)) OR @OtherFundingStatus IS NULL)
	 		--AND	(C.[AdultLearnerFundingStatus] IN (SELECT Data FROM [dbo].[Split](@AdultlrStatus, @splitChar)) OR @AdultlrStatus IS NULL)
			--AND	(C.ErAppStatus = @ErAppStatus OR @ErAppStatus IS NULL)
			--AND (C.SkillsForLife = @SkillForLifeFlg OR @SkillForLifeFlg IS NULL)
			AND	(CI.StudyModeBulkUploadRef IN (SELECT Data FROM [dbo].[Split](@StudyMode, @splitChar)) OR @StudyMode IS NULL)
			AND (CI.AttendanceModeBulkUploadRef IN (SELECT Data FROM [dbo].[Split](@AttendanceMode, @splitChar)) OR @AttendanceMode IS NULL)
			AND	(CI.AttendancePatternBulkUploadRef IN (SELECT Data FROM [dbo].[Split](@AttendancePattern, @splitChar)) OR @AttendancePattern IS NULL)
			AND	(CI.[ApplyUntilDate] < @AppClosed OR @AppClosed IS NULL)
			AND	(CI.CourseInstanceId IN (SELECT CourseInstanceId FROM CourseInstanceA10FundingCode WHERE A10FundingCode IN (SELECT Data FROM [dbo].[Split](@A10Code, @splitChar))) OR @A10Code IS NULL)
			AND	(CI.StartDate >= @EarliestStartDate OR @EarliestStartDate IS NULL OR @FlexiStartDate = 1)
			AND (CI.StartDate IS NULL OR CI.StartDate >= @OneYearAgo)
			AND (@StartPoint IS NOT NULL OR @Location IS NULL OR (@Location = V.Town OR V.County = @Location OR @Location = CI.RegionName OR V.Postcode LIKE @Location + '%'))
			AND (@Distance IS NULL OR @Distance >= CASE WHEN @StartPoint IS NULL OR V.Latitude IS NULL OR V.Longitude IS NULL THEN @DummyDistance
			...
			AND (CI.DfEFunded = @DFEFunded OR @DFEFunded IS NULL)
	)
